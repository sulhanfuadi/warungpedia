...
const [filter, setFilter] = useState<"ALL" | "ACTIVE" | "INACTIVE">("ALL");
  ...
  const handleDownloadReportByProvince = async () => {
    try {
      ...
      // Step 3: Buat HTML string (Landscape) - 4 kolom: No., Nama PIC, Nama Toko, Provinsi
      const now = new Date();
      const tanggalDibuat = now.toLocaleDateString("id-ID");
      
      let tableRows = "";
      reportData.data.forEach((seller: any, idx: number) => {
        tableRows += `<tr><td style="border:1px solid #000;padding:10px;text-align:center;color:#000;font-size:11px;">${idx + 1}</td><td style="border:1px solid #000;padding:10px;color:#000;font-size:11px;">${seller.pic_name || "-"}</td><td style="border:1px solid #000;padding:10px;color:#000;font-size:11px;">${seller.store_name || "-"}</td><td style="border:1px solid #000;padding:10px;text-align:center;color:#000;font-weight:bold;font-size:11px;">${seller.pic_city || "-"}</td></tr>`;
      });

      const htmlContent = `<html><head><meta charset="UTF-8"></head><body style="font-family:Arial,sans-serif;padding:15px;margin:0;color:#000;"><h1 style="text-align:center;color:#000;font-size:16px;margin:0 0 5px 0;">LAPORAN DAFTAR PENJUAL BERDASARKAN PROVINSI</h1><p style="text-align:center;color:#000;font-size:11px;margin:0 0 15px 0;">Warungpedia Admin System</p><div style="margin:10px 0;padding:10px;background:#f0f0f0;border:1px solid #000;"><p style="margin:3px 0;color:#000;font-size:11px;"><strong>Tanggal dibuat:</strong> ${tanggalDibuat} oleh ${adminName}</p><p style="margin:3px 0;color:#000;font-size:11px;"><strong>Total Penjual:</strong> ${reportData.data.length}</p></div><table style="width:100%;border-collapse:collapse;border:1px solid #000;margin:15px 0;"><thead><tr style="background:#0779FF;color:white;"><th style="border:1px solid #000;padding:10px;text-align:center;font-weight:bold;font-size:11px;">No.</th><th style="border:1px solid #000;padding:10px;text-align:left;font-weight:bold;font-size:11px;">Nama PIC</th><th style="border:1px solid #000;padding:10px;text-align:left;font-weight:bold;font-size:11px;">Nama Toko</th><th style="border:1px solid #000;padding:10px;text-align:center;font-weight:bold;font-size:11px;">Provinsi</th></tr></thead><tbody>${tableRows}</tbody></table><div style="margin-top:20px;border-top:1px solid #000;padding-top:10px;text-align:center;color:#000;font-size:9px;"><p style="margin:3px 0;">Dokumen ini dibuat otomatis oleh Warungpedia</p><p style="margin:0;">© 2025 Warungpedia</p></div></body></html>`;
